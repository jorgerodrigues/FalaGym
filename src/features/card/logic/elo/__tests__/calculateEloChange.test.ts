...
describe("calculateEloChange", () => {
  describe("Basic ELO calculations", () => {
    it("should calculate correct ELO change when user wins against equal opponent", () => {
      ...
      // Win (score=5) should give: 32 * (1 - 0.5) = 16 points
      const result = calculateEloChange(1200, 1200, SCORES.CORRECT);
    ...
    it("should calculate correct ELO change when user loses against equal opponent", () => {
      ...
      const result = calculateEloChange(1200, 1200, SCORES.INCORRECT);
      expect(result).toBeCloseTo(-16, 2);
    });
    ...
    it("should give smaller gains when beating weaker opponent", () => {
      ...
      const result = calculateEloChange(1400, 1200, SCORES.CORRECT);
    ...
    it("should give larger gains when beating stronger opponent", () => {
      ...
      const result = calculateEloChange(1200, 1400, SCORES.CORRECT);
    ...
    it("should give smaller losses when losing to stronger opponent", () => {
      ...
      const result = calculateEloChange(1200, 1400, SCORES.INCORRECT);
      expect(result).toBeGreaterThan(-16);
    ...
    it("should give larger losses when losing to weaker opponent", () => {
      ...
      const result = calculateEloChange(1400, 1200, SCORES.INCORRECT);
      expect(result).toBeLessThan(-16);
    });
  });
  ...
  describe("Edge cases", () => {
    it("should handle extreme ELO differences - much stronger user wins", () => {
      ...
      const result = calculateEloChange(1800, 1000, SCORES.CORRECT);
      expect(result).toBeGreaterThan(0);
    ...
    it("should handle extreme ELO differences - much weaker user wins", () => {
      ...
      const result = calculateEloChange(1000, 1800, SCORES.CORRECT);
      expect(result).toBeGreaterThan(25);
      expect(result).toBeLessThan(32);
    });

    it("should handle minimum ELO values", () => {
      const result = calculateEloChange(800, 800, SCORES.CORRECT);
      expect(result).toBeCloseTo(16, 2);
    });

    it("should handle maximum ELO values", () => {
      const result = calculateEloChange(2000, 2000, SCORES.INCORRECT);
      expect(result).toBeCloseTo(-16, 2);
    });

    it("should handle very large ELO differences", () => {
      // Test with extreme difference
      const result = calculateEloChange(800, 2000, SCORES.CORRECT);
      expect(result).toBeGreaterThan(0);
      expect(result).toBeLessThan(32);
    });
  });
  ...
  describe("Input validation", () => {
    it("should throw error for invalid actual score (not 0 or 5)", () => {
      expect(() => calculateEloChange(1200, 1200, 1)).toThrow(`actualScore must be ${SCORES.INCORRECT} or ${SCORES.CORRECT}`);
      expect(() => calculateEloChange(1200, 1200, 2)).toThrow(`actualScore must be ${SCORES.INCORRECT} or ${SCORES.CORRECT}`);
      expect(() => calculateEloChange(1200, 1200, -1)).toThrow(`actualScore must be ${SCORES.INCORRECT} or ${SCORES.CORRECT}`);
    });

    it("should accept valid actual scores", () => {
      expect(() => calculateEloChange(1200, 1200, SCORES.INCORRECT)).not.toThrow();
      expect(() => calculateEloChange(1200, 1200, SCORES.CORRECT)).not.toThrow();
    });
  });
  ...
  describe("Mathematical precision", () => {
    it("should calculate known ELO values correctly", () => {
      ...
      const result = calculateEloChange(1200, 1300, SCORES.CORRECT);
    ...
    it("should return zero change only when actual equals expected", () => {
      ...
      const winResult = calculateEloChange(1200, 1200, SCORES.CORRECT);
      const loseResult = calculateEloChange(1200, 1200, SCORES.INCORRECT);
    ...
    it("should maintain symmetry - win/loss should be equal magnitude for equal ELOs", () => {
      ...
      const winChange = calculateEloChange(userElo, sentenceElo, SCORES.CORRECT);
      const lossChange = calculateEloChange(userElo, sentenceElo, SCORES.INCORRECT);
  ...
  describe("Boundary testing", () => {
    it("should handle floating point ELO values", () => {
      const result = calculateEloChange(1200.5, 1199.7, SCORES.CORRECT);
    ...
    it("should handle negative ELO values", () => {
      ...
      const result = calculateEloChange(-100, 100, SCORES.CORRECT);
      expect(typeof result).toBe("number");
      expect(result).toBeGreaterThan(0);
    });
  });
